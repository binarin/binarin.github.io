...
As RabbitMQ is gradually adopting Elixir (i.e. for [[https://github.com/rabbitmq/rabbitmq-cli][the next generation
of its CLI tools]]), it's natural that one would also want to use Elixir
for writing plugins. There is a [[https://www.rabbitmq.com/blog/2013/06/03/using-elixir-to-write-rabbitmq-plugins/][blog post]] from 2013 on that topic and
...
#+HTML: <!--more-->
 ...
 be added to [[https://github.com/rabbitmq/rabbitmq-metronome/blob/master/Makefile][Makefile of the original metronome plugin]]:
 #+BEGIN_SRC makefile
   elixir_srcs := mix.exs \
 ...
 advisable to add some [[https://hexdocs.pm/mix/Mix.html#module-aliases][aliases]] to ~mix.exs~:
 #+BEGIN_SRC elixir
   [
     make_all: [
       "deps.get",
